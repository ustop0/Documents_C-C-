...
void login(const gchar *usuario_str, const gchar *senha_str, const gchar *lembrar_bool){
...
void bt_logar_clicked_cb(GtkWidget *widget, gpointer data){
    ...
    lembrar = GTK_WIDGET(gtk_builder_get_object(builder, "lembrar_usuario_check"));
    const gchar *lembrar_bool = gtk_toogle_button_active(lembrar); //check button -> WIDGET
    
    //chama a função responsável pelo login do sistema
    login(usuario_str, senha_str, lembrar_bool);
}
