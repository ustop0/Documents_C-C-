...
void on_botao_clicked(GtkWidget *widget, gpointer data){
    ...
    if(imc > 27){
        char estado_imc[] = "Você está obeso";
        g_print("%s", estado_imc);
        
        gtk_label_set_text(estado, estado_imc);
    }
}
...
int main(int argc, char *argv[]) {
	...
	GtkBuilder *builder = gtk_builder_new_from_file("IMC.glade");
    ...
    estado = GTK_LABEL(gtk_builder_get_object(builder, "estado_label"));
